...
public class SimpleCacheService {
    ...
    public <T> List<T> lGetRange(String key, Class<T> clazz, int start, int end) {
        List<T> list = new ArrayList();
        if (!StringUtils.isEmpty(key)) {
            for (String s : redisTemplate.opsForList().range(key, start, end)) {
                T a = JSON.parseObject(s, clazz);
                list.add(a);
            }
        }
        return list;
